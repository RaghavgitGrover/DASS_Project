...
const ViewTimeTables = () => {
    ...
    const formatSlotTime = (slotTimes, slotKey) => {
        if (!slotTimes || !slotTimes[slotKey]) return '';
        const s = slotTimes[slotKey];
        if (!s.startHour || !s.endHour) return '';
        return `: ${s.startHour}:${s.startMin} ${s.startPeriod} to ${s.endHour}:${s.endMin} ${s.endPeriod}`;
      };
        ...
        try {
            ...
            if (timetable.data?.timetable?.timetable) {
                ...
                Object.keys(tt).sort().forEach(day => {
                    ...
                    const dateStr = timetable.data?.examDates?.[index]
                        ? new Date(timetable.data.examDates[index]).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })
                        : '';
                    const dayRow = [`Day ${index + 1}${dateStr ? `: ${dateStr}` : ''}`];

                    
                    // For each slot (up to 4)
                    const maxSlots = timetable.data?.examType === 'endsem' ? 2 : 4;
            ...
            const slotColWidth = remainingWidth / maxSlots;
            ...
            autoTable(doc, {
                head: [
                    timetable.data?.examType === 'endsem'
                      ? [
                          'Day',
                          `Slot 1${formatSlotTime(timetable.data?.slotTimes, 'slot-1')}`,
                          `Slot 2${formatSlotTime(timetable.data?.slotTimes, 'slot-2')}`
                        ]
                      : [
                          'Day',
                          `Slot 1${formatSlotTime(timetable.data?.slotTimes, 'slot-1')}`,
                          `Slot 2${formatSlotTime(timetable.data?.slotTimes, 'slot-2')}`,
                          `Slot 3${formatSlotTime(timetable.data?.slotTimes, 'slot-3')}`,
                          `Slot 4${formatSlotTime(timetable.data?.slotTimes, 'slot-4')}`
                        ]
                  ],
            ...
            <div className="overflow-x-auto">
                ...
                <thead className="bg-gray-50">
                    <tr>
                        ...
                        {(() => {
                            const slotTimes = selectedTimetable.data?.slotTimes || {};
                            const renderSlotLabel = (slotKey, index) => {
                                const s = slotTimes[slotKey];
                                if (s?.startHour && s?.endHour) {
                                    return `Slot ${index}: ${s.startHour}:${s.startMin} ${s.startPeriod} to ${s.endHour}:${s.endMin} ${s.endPeriod}`;
                                }
                                return `Slot ${index}`;
                            };

                            return (isEndsem ? [1, 2] : [1, 2, 3, 4]).map((i) => (
                                <th key={i} className="border p-2 font-semibold">
                                    {renderSlotLabel(`slot-${i}`, i)}
                                </th>
                            ));
                        })()}
                    </tr>
