...
const ViewInvigilation = () => {
  ...
  const fetchAssignments = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:3000/api/saved-invigilation-assignments');
  ...
  const handleDeleteAssignment = async (assignmentId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/saved-invigilation-assignments/${assignmentId}`, {
        method: 'DELETE',
      });
  ...
  const handleSendEmails = async () => {
    try {
      ...
      // Process faculty names (IDs 52-165)
      Object.entries(selectedAssignmentForEmail.dutySummary.faculty).forEach(([name, _], index) => {
        const id = (52 + index).toString(); // Start from 52 for faculty
        nameToIdMap.set(name, id);
      });

      // Process staff names (IDs 1-51)
      Object.entries(selectedAssignmentForEmail.dutySummary.staff).forEach(([name, _], index) => {
        const id = (1 + index).toString(); // Start from 1 for staff
        nameToIdMap.set(name, id);
      });

      console.log('Name to ID mapping:', Object.fromEntries(nameToIdMap));
      ...
      dates.forEach((date, dayIndex) => {
        ...
        console.log(`Using day key: ${dayKey}`);

        const daySlots = selectedAssignmentForEmail.invigilation[dayKey] || {};
        ...
        Object.entries(daySlots).forEach(([slot, rooms]) => {
          ...
          if (!slotTime) {
            console.error(`No time found for slot ${slotNumber}`);
            return;
          }
          console.log('Slot time:', slotTime);
          console.log('Rooms for this slot:', rooms);

          Object.entries(rooms).forEach(([roomId, roomData]) => {
            console.log('Room data:', roomData);
            console.log('Invigilators for this room:', roomData.invigilators);

            roomData.invigilators.forEach(invigilator => {
              console.log('Processing invigilator:', invigilator);
              const id = nameToIdMap.get(invigilator.name);

              if (!id) {
                console.error(`Could not find ID for invigilator: ${invigilator.name}`);
                return;
              }

              console.log(`Found ID ${id} for ${invigilator.name}`);
