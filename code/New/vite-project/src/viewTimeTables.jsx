...
const ViewTimeTables = () => {
            ...
            for (const timetable of timetableList) {
                ...
                try {
                    ...
                    if (data.success && data.data) {
                        validTimetables.push(data.data);
                    } else if (data.id) {
                        // Legacy format
        ...
        try {
            ...
            if (timetable.data?.timetable?.timetable) {
                ...
                Object.keys(tt).sort().forEach(day => {
                    ...
                    const totalSlots = timetable.data?.examType === 'endsem' ? 2 : 4;
            ...
            const slotColWidth = remainingWidth / totalSlots;


            autoTable(doc, {
                head: timetable.data?.examType === 'endsem'
                    ? [['Day', 'Slot 1', 'Slot 2']]
    ...
    try {
        ...
        // Header row
        if (timetable.data?.examType === 'endsem') {
            wsData.push(['Day/Date', 'Slot 1', 'Slot 2']);
          } else {
    ...
    const handleDeleteTimetable = async (id) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/timetable_delete/${id}`, {
                method: 'DELETE'
            });
            ...
            // Update the local state to remove the deleted timetable
            setTimetables(prev => prev.filter(t => t.id !== id));
            
            // Close the modal if the deleted timetable was selected
            if (selectedTimetable && selectedTimetable.id === id) {
